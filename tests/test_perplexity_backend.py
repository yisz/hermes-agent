...
@pytest.fixture()
def provider():
    """Return a fresh PerplexityWebSearchProvider instance."""
...
class TestPerplexityWebSearchProvider:
    """Tests for PerplexityWebSearchProvider."""
    ...
    def test_does_not_support_extract(self, provider):
        assert provider.supports_extract() is False

    def test_is_available_with_key(self, provider, perplexity_key):
...
class TestPerplexitySearch:
    """Tests for PerplexityWebSearchProvider.search() with mocked HTTP calls."""

    def test_search_returns_results(self, provider, perplexity_key, monkeypatch):
        mock_resp = MagicMock()
        ...
        with patch("plugins.web.perplexity.provider.httpx.post", return_value=mock_resp):
            result = provider.search("machine learning", limit=5)
    ...
    def test_search_sends_correct_payload(self, provider, perplexity_key, monkeypatch):
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = {"id": "t", "results": []}
        mock_resp.raise_for_status = MagicMock()

        with patch("plugins.web.perplexity.provider.httpx.post", return_value=mock_resp) as mock_post:
            provider.search("test query", limit=10)
    ...
    def test_search_limit_clamped_to_20(self, provider, perplexity_key, monkeypatch):
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = {"id": "t", "results": []}
        mock_resp.raise_for_status = MagicMock()
    ...
    def test_search_limit_floor_at_1(self, provider, perplexity_key, monkeypatch):
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.json.return_value = {"id": "t", "results": []}
        mock_resp.raise_for_status = MagicMock()
    ...
    def test_no_api_key_returns_error(self, provider, monkeypatch):
    ...
    def test_401_auth_error(self, provider, perplexity_key, monkeypatch):
        error_resp = MagicMock()
        error_resp.status_code = 401
        error_resp.text = "Unauthorized"
        error_resp.headers = {}

        with patch("plugins.web.perplexity.provider.httpx.post") as mock_post:
    ...
    def test_429_rate_limit_error(self, provider, perplexity_key, monkeypatch):
        error_resp = MagicMock()
    ...
    def test_429_without_retry_after(self, provider, perplexity_key, monkeypatch):
        error_resp = MagicMock()
        error_resp.status_code = 429
    ...
    def test_500_server_error(self, provider, perplexity_key, monkeypatch):
        error_resp = MagicMock()
    ...
    def test_timeout_error(self, provider, perplexity_key, monkeypatch):
        with patch("plugins.web.perplexity.provider.httpx.post") as mock_post:
    ...
    def test_connection_error(self, provider, perplexity_key, monkeypatch):
        with patch("plugins.web.perplexity.provider.httpx.post") as mock_post:
    ...
    def test_invalid_json_response(self, provider, perplexity_key, monkeypatch):
        mock_resp = MagicMock()
        mock_resp.status_code = 200
        mock_resp.raise_for_status = MagicMock()
    ...
    def test_interrupted_returns_error(self, provider, perplexity_key, monkeypatch):
        with patch("tools.interrupt.is_interrupted", return_value=True):
            result = provider.search("test")
        assert result["success"] is False
        assert "Interrupted" in result["error"]

    def test_custom_api_url(self, provider, perplexity_key, monkeypatch):
    ...
    def test_no_max_tokens_per_page_in_payload(self, provider, perplexity_key, monkeypatch):
